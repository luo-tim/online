try{
    ...
    sf[35]=new Array("省份","城市"); 
}catch(e){
	alert(e);	
}
function getQueryVariable(variable) {//获取url中的参数
	var s=location.href;
	s=s.replace("?","?&").split("&");
	var re="";
	for (i = 1;i<s.length;i++){
		if(s[i].indexOf(variable+"=")==0){
			re=s[i].replace(variable+"=","");
		}
	}
	return re;
}
function setAddress() {
	try {
		...
		$("#query").val(query);
		var pro =decodeURI(getQueryVariable("pro"));
		//console.log(pro);
		var city=decodeURI(getQueryVariable("city"));
		var type=decodeURI(getQueryVariable("type"));
		console.log(type);
		$("#type").val(type);
		...
		for (i = 0; i < sf.length; i++) {
			if (pro == sf[i][0].toString()) {
				...
				for (j = 0; j < tmpcity.length; j++) {
					if(city==tmpcity[j]){
						$("#city").append("<option selected>" + tmpcity[j] + "</option>");
					}else{
...
function postWorkerForm(){

    let form=document.workerForm;
    let jobName=form.jobName.value;
  
    let jobIntroduce=form.jobIntroduce.value;
    let jobSalary=form.jobSalary.value;
    let city=form.city.value;
    let jobAddress=form.jobAddress.value;
    let jobRequest=form.jobRequest.value;
